...
def simulate_overlap(dataset_dir, cur_id, overlap_size=160, simulate_num = 50,
    ...
    overlap_dir = os.path.join(dataset_dir, cur_id + "overlap")
    ...
    while success_num < simulate_num:
        ...
        overlap_img = np.zeros((overlap_size, overlap_size), np.uint16)
        ...
        overlap_img[rand_h1:rand_h1+h1, rand_w1:rand_w1+w1] += img1
        overlap_img[rand_h2:rand_h2+h2, rand_w2:rand_w2+w2] += img2
...
if __name__ == "__main__":
    ...
    img_ids = [str(id) for id in np.arange(1, 120)]
